...
public static class NyangbingoDevARegressionTests
{
    ...
    private static void TestForegroundBackgroundPlacementContracts(WorldGenerationConfig config)
    {
        ...
        try
        {
            ...
            Assert(tiles.SupportsForegroundPlacement(WorldTileTypes.Dirt), "dirt가 전경 설치 가능이어야 함");
            Assert(tiles.SupportsForegroundPlacement(WorldTileTypes.Stone), "stone이 전경 설치 가능이어야 함");
            Assert(!tiles.SupportsForegroundPlacement(WorldTileTypes.Bedrock), "기반암은 전경 재설치 불가");
            Assert(!tiles.SupportsForegroundPlacement(WorldTileTypes.IceAltar), "제단은 전경 재설치 불가");
            Assert(!tiles.SupportsForegroundPlacement(WorldTileTypes.BackgroundDirt), "배경 ID는 전경 설치 불가");
            Assert(!tiles.SupportsForegroundPlacement(WorldTileTypes.Wallpaper), "벽지는 전경 설치 불가");
